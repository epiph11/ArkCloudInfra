...
**Azure — toujours bloqué, pour une raison différente** : le Runbook Automation actuel n'a besoin d'aucun accès réseau aujourd'hui, parce qu'il ne fait que des appels à l'API de gestion (`Update-AzPostgresFlexibleServer -AdministratorLoginPassword`), jamais de connexion SQL directe. Étendre ce Runbook pour exécuter du SQL réel (`ALTER ROLE`) demande donc de lui donner un chemin réseau vers `vnet-arkcloud-dev` qu'il n'a pas — concrètement un Hybrid Runbook Worker (VM ou conteneur dans le VNet) ou l'intégration VNet native des comptes Automation. Pas encore construit ; tant que ce n'est pas fait, il n'existe aucun moyen d'exécuter `CREATE ROLE`/`ALTER ROLE` contre le Postgres Azure depuis l'extérieur du VNet.
...
Côté Azure, toujours bloqué (Runbook sans accès réseau, voir ci-dessus). `ConnectionStrings--DefaultConnection` reste sur `arkcloudadmin` sur les deux clouds — le rôle AWS existe mais rien ne l'utilise encore : la bascule (étape 4 du découpage, `docs/infra-roadmap.md`) attend que le rôle équivalent existe aussi côté Azure, pour faire le changement une seule fois sur les deux clouds plutôt que de désynchroniser les environnements.
