...
**Reproductibilité du build Lambda entre local (Windows/Git Bash) et CI (Ubuntu)** : `filebase64sha256()` sur le zip signifie que deux constructions du même code doivent produire le même hash, sous peine de voir Terraform considérer la Lambda comme modifiée à chaque alternance local/CI — même classe de problème que le flip-flop de security group ci-dessus. `build.sh` neutralise trois sources de non-déterminisme (version de dépendance épinglée, dates de fichiers figées, ordre + attributs de zip normalisés via un archiveur Python plutôt que le binaire `zip`/`Compress-Archive`) et génère un manifeste par fichier (`build/manifest.txt`, publié en CI dans le Summary du job) pour localiser toute divergence future au lieu de la deviner. Une hypothèse initiale (fins de ligne CRLF sur `rotate.py` côté Windows) s'est révélée non déterminante — le hash local n'a pas changé après le fix `tr -d '\r'`, ce qui a été traité comme un résultat réel plutôt qu'ignoré. La preuve de reproductibilité retenue n'est pas la comparaison manuelle de deux hashes mais le `terraform plan` lui-même : un plan qui reste vide après un build CI, sur un état posé par un apply local, démontre que les deux constructions produisent un zip identique — sans quoi Terraform afficherait une mise à jour en place de `aws_lambda_function.rotation`. Confirmé ainsi fin août 2026 ; la cause exacte du tout premier écart observé (avant l'archiveur Python déterministe) reste non identifiée avec certitude, mais sans conséquence puisque le comportement observable — le seul qui compte pour Terraform — est maintenant stable.
