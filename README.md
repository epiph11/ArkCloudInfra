...
- **`CKV_AZURE_65`/`CKV_AZURE_66`** (detailed error messages / failed request tracing) — le bloc `logs { detailed_error_messages { enabled = true } failed_request_tracing { enabled = true } }` est bien présent dans `modules/azure/app-service/main.tf` (syntaxe correcte, conforme au schéma actuel d'`azurerm_linux_web_app`) — mais ces deux checks ont été écrits à l'origine pour l'ancienne ressource dépréciée `azurerm_app_service`, pensée pour du diagnostic Windows/IIS. Ils ne matchent pas fiablement la variante Linux de la ressource — un problème de détection côté Checkov, pas un vrai manque sur un App Service Linux conteneurisé où ce type de diagnostic IIS n'a de toute façon pas vraiment de sens.
