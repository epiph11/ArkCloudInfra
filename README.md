...
### Sprint 6 — Logs d'accès ALB (remédiation STRIDE "Repudiation", flux 1)

`CKV_AWS_91` (ALB access logging) avait été écarté au premier passage Checkov du Sprint 6 faute de bucket S3 dédié. Repris comme remédiation directe du threat model STRIDE (`docs/threat-model-stride.md`, ArkCloud) : sans ça, rien ne permettait de prouver après coup qui avait atteint le service au niveau infra, seule l'application logue ses propres requêtes.

Bucket dédié (`modules/aws/alb`) : `BucketOwnerEnforced`, chiffrement SSE-S3 (AES256 — **seule option supportée** par la delivery ELB, une clé KMS ferait échouer la livraison silencieusement), versioning, expiration à 90 jours (même fenêtre que CloudTrail et les flow logs Azure). `access_logs` activé sur `aws_lb.this`.

**Deux vrais bugs rencontrés au premier apply, ni l'un ni l'autre une erreur de frappe** :
- **Délai de propagation supposé, écarté par la preuve** : premier `Access Denied for bucket` mis sur le compte d'une propagation IAM/S3 pas encore effective — un retry (plusieurs minutes après, donc largement au-delà d'un simple délai) a échoué exactement pareil, invalidant cette hypothèse.
- **Cause réelle : une condition `aws:SourceArn` (via `ArnLike`) ajoutée par prudence, absente de la policy de référence AWS et d'une configuration Terraform tierce vérifiée comme fonctionnelle.** IAM évalue une condition qui référence une clé de contexte absente de la requête comme **fausse**, pas comme ignorée — si le service de délivery ELB ne peuplait pas systématiquement `aws:SourceArn` sur cet appel `PutObject`, la condition transformait silencieusement tout le `Allow` en no-op. Retirée : le chemin de ressource (`.../AWSLogs/<compte>/*`) restreint déjà l'accès à ce compte, sans avoir besoin d'une condition non documentée dans la policy de référence.

**Vérifié de bout en bout, pas juste par l'absence d'erreur Terraform** : requête réelle envoyée à l'ALB (`curl.exe -k`, certificat auto-signé — voir plus haut), fichier de validation `ELBAccessLogTestFile` d'AWS confirmé présent dans le bucket, puis plusieurs objets `.log.gz` livrés en continu (`aws s3 ls --recursive`), toutes les 5-10 minutes comme attendu.
...
### Sprint 6 — `modules/azure/functions-experiment` (ADR-0010), 5 findings

- **`CKV_AZURE_190`/`CKV2_AZURE_47`** (accès public/anonyme aux blobs), **`CKV2_AZURE_38`** (soft-delete), **`CKV2_AZURE_21`** (logging lecture Blob) — les 4 portent sur le storage account de ce module, qui ne contient que le zip de déploiement de la fonction Python (aucun secret, aucune donnée applicative — les vrais secrets restent dans Key Vault). ADR-0010 a déjà tranché : ce module est un mécanisme temporaire d'expérimentation, remplacé à terme par la procédure Kudu (backlog, tâche #83) et destiné à être détruit plutôt que conservé — durcir un storage account qu'on prévoit de supprimer serait du travail jeté.
- **`CKV2_AZURE_31`** (subnet sans NSG) sur `module.network.azurerm_subnet.functions[0]` — faux positif confirmé en lisant le code : `azurerm_subnet_network_security_group_association.functions` existe bien (`modules/azure/network/main.tf`), avec le même `count` conditionnel que le subnet lui-même. Checkov ne résout pas cette association à travers un index de `count` sur une ressource conditionnelle. Même classe de limitation que `CKV_AZURE_43`/`CKV_AWS_304` documentés plus haut, pas un vrai manque réseau.

---

## 10. Rotation des secrets — procédure manuelle, rappels automatisés (Sprint 6)

Rotation elle-même toujours manuelle pour `POSTGRES_ADMIN_PASSWORD` (routine, pas d'échéance dure) et pour `Jwt:Key` (voir plus bas pourquoi) — l'automatiser proprement demanderait un vrai pipeline de renouvellement côté Key Vault/Secrets Manager (versions de secrets + redéploiement déclenché par le changement de version), disproportionné pour un environnement dev à ce stade. **Correction (Sprint 6, en cours)** : le mot de passe Postgres lui-même tourne désormais automatiquement tous les 90 jours sur les deux clouds (voir plus bas, "Rotation automatique des mots de passe Postgres") — ce qui reste manuel pour ce secret, c'est uniquement la mise à jour de `last_rotated` dans l'inventaire, pas la rotation elle-même. Ce qui **est** automatisé depuis le Sprint 6 : plus besoin de se souvenir des échéances — `.github/workflows/secret-expiry-check.yml` tourne tous les jours (+ sur chaque modif de l'inventaire) et fait échouer le run si un secret à échéance dure approche de l'expiration, ou si un secret sans échéance dure n'a pas été tourné depuis trop longtemps. Source de vérité : `.github/secrets-inventory.json` — à mettre à jour à chaque rotation réelle, sinon le check continue de réclamer une rotation déjà faite.

**`GHCR_PAT` — étapes 2 à 5 automatisées (Sprint 6)** : générer le nouveau token reste un humain sur github.com/settings/tokens (étape 1, aucune API GitHub ne le permet), mais `scripts/rotate-ghcr-pat.ps1` enchaîne le reste : `gh secret set` → déclenche un vrai `terraform apply` → attend qu'il finisse → `az webapp restart` sur les deux App Services → met à jour `expires_on` dans `secrets-inventory.json` → commit + push. Usage : `./scripts/rotate-ghcr-pat.ps1 -ExpiresOn yyyy-MM-dd` (prompt caché pour le token, jamais écrit sur disque ni loggé).

**Bug réel rencontré lors d'une rotation (Sprint 6)** : après un `terraform apply` confirmé (`docker_registry_password` détecté comme changé, poussé sans erreur) puis un restart, `app-arkcloud-api-dev` continuait à échouer avec `ImagePullUnauthorizedFailure` — persistant même après un nouvel apply qui affichait ensuite `No changes` (l'état correspondait bien à la config voulue). Seul un `az webapp config container set` en CLI (API différente, plus ancienne) a réellement débloqué le pull. Root-caused, pas supposé : bug confirmé du provider `azurerm` ([`hashicorp/terraform-provider-azurerm#22996`](https://github.com/hashicorp/terraform-provider-azurerm/issues/22996), même classe que `#23525`) — les arguments `application_stack.docker_registry_*` ne se propagent pas de façon fiable au sous-système réel de pull de l'App Service.

**Tentative de correctif dans Terraform, abandonnée** : poser aussi les trois `app_settings` classiques (`DOCKER_REGISTRY_SERVER_URL`/`_USERNAME`/`_PASSWORD`) en plus du bloc `application_stack` a été essayé — `terraform plan` le rejette explicitement (`cannot set a value for DOCKER_REGISTRY_SERVER_PASSWORD in app_settings`) : le provider interdit ces trois clés dans `app_settings` dès qu'`application_stack` est utilisé, exactement l'inverse de ce que suggérait une lecture rapide de l'issue amont. Le contournement `az webapp config container set` reste donc **manuel**, exécuté après chaque rotation — `scripts/rotate-ghcr-pat.ps1` l'appelle désormais automatiquement (étape 4bis) sur les deux App Services, juste après le restart.

Ça a nécessité un vrai correctif préalable : le job `apply` de `terraform-ci.yml` ne tournait que sur un push vers `main` (`if: github.event_name == 'push' && ...`) — un déclenchement manuel (`workflow_dispatch`, ajouté en tâche #59) ne faisait tourner que `validate` (plan), jamais l'apply réel. La procédure manuelle documentée ci-dessous supposait à tort qu'un simple re-run suffisait à repropager un secret déjà changé — ce n'était vrai pour aucun des deux workflows tant que cette condition n'a pas été élargie à `|| github.event_name == 'workflow_dispatch'`. Sans ce correctif, ni la procédure manuelle ni ce script n'auraient réellement fonctionné.
...
### Clés partagées Log Analytics (`log-arkcloud-dev`) — non régénérées, risque accepté (Sprint 6)

Tentative de régénération par précaution (suite aux tâches #86/#87, purge/rotation après une fuite de tfplan) — sans succès, et volontairement abandonnée plutôt que poursuivie. Constaté, pas supposé :
- Le portail Azure ne propose plus ce contrôle pour ce workspace : ni `Classic > Agents` (page redessinée vers Azure Monitor Agent/Data Collection Rules, section clés disparue), ni `Properties` (Workspace ID uniquement, pas de clés).
- `az monitor log-analytics workspace` n'a pas de sous-commande `regenerate-shared-keys`. L'appel direct à l'API ARM (`POST .../regenerateSharedKey?api-version=2023-09-01`) échoue systématiquement avec `InvalidParameter: keyType value must equal primarySharedKey or secondarySharedKey` — y compris avec un corps JSON confirmé valide (vérifié via fichier local, `Get-Content` relu avant l'appel). L'opération semble ne plus être fonctionnelle en libre-service pour ce type de ressource.

**Décision** : pas de ticket de support Microsoft ouvert pour ce point — risque résiduel nul vérifié (`grep` sur tout le repo : aucune référence à `shared_key`/`primary_shared_key`/`workspace_key`, "0 Linux computers connected" sur la page Agents, aucun agent MMA/OMS legacy connecté à ce workspace). Rien dans ce projet ne lit ni n'écrit cette clé. Rotation via ticket resterait possible si un jour un vrai consommateur de cette clé apparaît.
...
### Rôle applicatif `arkcloud_app` — remédiation STRIDE "Elevation of privilege" (Sprint 6, en cours)

`ArkCloud.API` se connecte aujourd'hui à Postgres comme `arkcloudadmin`, le compte admin du serveur — bien plus de droits que nécessaire pour du CRUD applicatif (voir `docs/threat-model-stride.md`, flux 3, ArkCloud repo). Objectif : un rôle dédié, DML seul (`SELECT`/`INSERT`/`UPDATE`/`DELETE`, jamais de `CREATE`/`DROP`/`ALTER` sur le schéma), avec un mécanisme de rotation propre plutôt qu'un mot de passe posé une fois à la main et jamais retouché.

**Découverte qui a changé le plan initial** : le premier essai (`scripts/bootstrap-app-role.ps1`, exécuté en local via `psql`) a échoué avec une erreur DNS (`Name or service not known`) — pas un problème d'identifiants. Cause réelle, confirmée par `az postgres flexible-server show`/`describe-db-instances` sur les deux clouds : les deux serveurs Postgres sont en accès strictement privé (`publicNetworkAccess: Disabled` + `delegatedSubnet` côté Azure, `publicly_accessible: false` côté AWS RDS) — aucune machine hors du VNet/VPC ne peut les joindre, quel que soit le mot de passe. `bootstrap-app-role.ps1` ne pouvait donc jamais fonctionner tel qu'écrit.

**AWS — résolu en fusionnant bootstrap et rotation, pas en cherchant un accès réseau séparé** : la Lambda de rotation existante (`modules/aws/secret-rotation`) tourne déjà dans le VPC pour la rotation du mot de passe master — donc capable, sans rien construire de nouveau, d'atteindre RDS en SQL. Le module a été étendu avec une variable `target_role` (`"master"`, comportement inchangé, ou `"app"`) : en mode `app`, `lambda/rotate.py` se connecte comme `arkcloudadmin` (lecture seule du secret admin existant, jamais réécrit) et exécute directement `CREATE ROLE`/`ALTER ROLE`/`GRANT` plutôt que d'appeler l'API de gestion RDS (qui ne peut de toute façon rotationner que le compte master désigné — pas un rôle Postgres arbitraire). `environments/dev/main.tf` instancie ce mode via `module.aws_secret_rotation_app_role`, sur un nouveau secret Secrets Manager (`arkcloud/arkcloud-dev/arkcloud-app-role`, `modules/aws/secrets`) seedé avec une valeur jetable puis immédiatement écrasée par le premier run de la Lambda. Attacher la rotation déclenche un premier run immédiat (même mécanisme que le secret master) — donc un seul `terraform apply` crée le rôle **et** pose son premier vrai mot de passe, sans script séparé à lancer à la main. Détail complet : `modules/aws/secret-rotation/lambda/README.md`.

**Azure — résolu via Azure Functions (Sprint 6), pas via le chemin initialement prévu** : le Runbook Automation actuel n'a besoin d'aucun accès réseau aujourd'hui, parce qu'il ne fait que des appels à l'API de gestion (`Update-AzPostgresFlexibleServer -AdministratorLoginPassword`), jamais de connexion SQL directe — l'étendre pour exécuter du SQL réel demanderait un Hybrid Runbook Worker (VM continue, ~15-70 $/mois pour une tâche trimestrielle, écarté sur ce seul critère coût/bénéfice — voir ADR-0010). À la place, `modules/azure/functions-experiment` (Function App Python, Flex Consumption, identité managée) exécute le bootstrap/rotation directement en SQL — `CREATE ROLE`/`ALTER ROLE`/`GRANT`, testé en conditions réelles, mot de passe vérifié dans Key Vault. **Correction du 07/09/2026** : l'ADR-0010 avait initialement retenu Kudu SSH comme solution *définitive* et documenté ce module comme temporaire/à démonter — en pratique il n'a jamais été démonté et reste le mécanisme réel utilisé aujourd'hui, Kudu n'ayant jamais été implémenté (voir addendum ADR-0010 : Kudu sur un conteneur custom demande d'ajouter `sshd` au Dockerfile, jamais fait, reporté en backlog). Invocation manuelle via `curl` avec la clé de fonction — voir `modules/azure/functions-experiment/README.md`. **Mise à jour du 11/09/2026** : Kudu a finalement été implémenté (`sshd` ajouté au Dockerfile de `ArkCloud.API`), vérifié en conditions réelles (session SSH établie sur `app-arkcloud-api-dev`, `psql` fonctionnel), et une rotation complète d'`arkcloud_app` a été effectuée avec succès par cette voie — `ALTER ROLE`/`GRANT` via Kudu+psql, Key Vault mis à jour, `/health` confirmé 200. Kudu est donc devenu le mécanisme réel, et `modules/azure/functions-experiment` a été démonté (`terraform destroy -target`, retiré d'`environments/dev/main.tf`) — code source conservé dans le repo pour mémoire, voir son README pour le détail. Procédure de rotation à jour : `docs/runbooks/rotate-arkcloud-app-azure-kudu.md`.

**État à date** : côté AWS, appliqué et vérifié en conditions réelles, pas juste "le plan Terraform est vert". `terraform apply` (11 ressources créées, 2 modifiées, 0 détruites) a déclenché la première rotation immédiatement, et les logs CloudWatch de `secret-rotation-arkcloud-dev-app` confirment l'enchaînement complet des quatre étapes : `createSecret: staged a new connection string as AWSPENDING` → `setSecret: created role arkcloud_app (bootstrap, first rotation)` puis `role arkcloud_app ready with DML-only grants` → `testSecret: pending credential authenticated successfully` (le rôle s'authentifie réellement avec son nouveau mot de passe) → `finishSecret: promoted ... to AWSCURRENT`, `no ECS_SERVICE configured for this role, skipping redeploy` (comportement voulu, rien ne consomme encore ce secret). Le rôle `arkcloud_app` existe donc réellement sur RDS aujourd'hui, avec un mot de passe géré automatiquement.

**Mise à jour** : le rôle équivalent a depuis été créé côté Azure via le Function App décrit ci-dessus, et la bascule réelle de `ConnectionStrings--DefaultConnection` vers `arkcloud_app` a été faite sur les deux clouds simultanément (pas désynchronisée entre environnements) — `arkcloudadmin` n'est plus utilisé par le trafic applicatif courant sur aucun des deux clouds.
...
- **Rôle IAM AWS `arkcloudinfra-ci` : `AdministratorAccess`** — pire que "non documenté" : l'identité qui fait tourner `terraform apply` sur `environments/dev` avait un accès admin complet au compte AWS, sans aucun rapport avec ce qu'elle gère réellement. Corrigé en deux policies IAM personnalisées, construites à partir d'un inventaire réel des 47 types de ressources `aws_*` que ce Terraform gère (pas deviné) — une seule policy dépassait la limite AWS de 6144 caractères (customer-managed policy), d'où le split :
  ...
  **Deux bugs réels découverts après coup, à l'occasion d'un `plan` ultérieur (Sprint 6, suite)** :
  - **AWS** : les deux policies ci-dessus scopaient certaines actions par préfixe de nom (`arkcloud-dev-*`), une convention copiée d'Azure sans être vérifiée côté AWS — fausse en pratique, chaque module AWS a sa propre convention de nommage (`ecs-execution-arkcloud-dev`, `pg16-arkcloud-dev-...`, `arkcloud/arkcloud-dev/postgres-...`, etc.). Un `terraform plan` a échoué en cascade avec des `AccessDenied` sur la quasi-totalité des ressources AWS en essayant de relire l'état existant. Corrigé en abandonnant le filtre par nom et en gardant compte (`386275436389`) + région (`eu-west-1`) comme seule frontière de scoping — moins granulaire qu'un préfixe de nom exact, mais fiable, contrairement à une convention supposée puis démentie par la réalité.
  - **Azure** : conséquence directe du resserrement du rôle CI décrit juste au-dessus (souscription → `rg-arkcloud-dev`). Deux ressources gérées par ce Terraform vivent hors de ce resource group : Defender for Cloud (`Microsoft.Security/pricings`, scopé à la souscription entière — déjà documenté comme exception plus haut dans ce README, mais la permission correspondante n'avait pas suivi) et le Network Watcher (`NetworkWatcherRG`, resource group auto-géré par Azure). Corrigé avec deux rôles personnalisés supplémentaires, chacun scopé au strict nécessaire — `arkcloud-ci-security-center` (souscription, `Microsoft.Security/pricings/{read,write}`) et `arkcloud-ci-network-watcher` (`NetworkWatcherRG`, lecture + gestion des flow logs).

  **Suite réelle, en CI (pas juste en local)** : le `plan` local propre ci-dessus ne racontait pas toute l'histoire — le tout premier run CI après avoir vraiment détaché `AdministratorAccess` a révélé, en plusieurs rounds successifs, une longue liste d'actions de lecture manquantes que le `plan` local n'avait pas toutes déclenchées (state déjà en cache localement vs. `terraform init`/refresh complet à froid en CI). Trois rounds, chacun corrigé contre l'erreur réelle plutôt que deviné à l'avance :
  1. `ec2:DescribeAddressesAttribute`, `ec2:DescribePrefixLists` (EIP/VPC endpoint), `events:ListTagsForResource` (règle EventBridge GuardDuty) — actions simplement oubliées à l'écriture initiale.
  2. `s3:GetBucketAcl`, `sns:ListTagsForResource`, `lambda:GetFunctionCodeSigningConfig` — même cause, un round plus tard.
  3. `s3:GetBucketCORS` puis, plutôt que de continuer action par action, généralisé en `s3:Get*`/`s3:List*` (toujours en lecture seule, toujours gardé par la condition `s3:ResourceAccount` du compte) — `aws_s3_bucket` rafraîchit à lui seul une dizaine de sous-configurations (ACL, CORS, versioning, chiffrement, logging, website, replication...) à chaque plan, corriger un `Get*` à la fois n'était plus le bon niveau d'effort.

  Reconfirmé par un `terraform plan` CI propre après ces trois rounds : `0 to add, 2 to change, 0 to destroy`, mêmes deux diffs `docker_registry_password`, plus aucune erreur `AccessDenied`.
...
**ArchUnitNET — vérifié depuis, un vrai bug trouvé et corrigé** : le premier `dotnet test` réel sur `ArkCloud.Tests.Architecture` (après avoir été écrit sans accès `dotnet` ni réseau) a révélé que 12 des 12 règles échouaient — `ResideInAssembly("ArkCloud.Domain")` etc. matchait silencieusement zéro type. Root cause confirmée par un test de diagnostic temporaire (dump de `Architecture.Types.Select(t => t.Assembly.Name)`, qui montrait les bons noms d'assembly alors que le prédicat renvoyait quand même un ensemble vide) : la surcharge `ResideInAssembly(string)` matche l'identité complète de l'assembly (nom + version + culture + token), pas le nom court. Corrigé en passant directement l'objet `System.Reflection.Assembly` déjà chargé plutôt qu'une chaîne. Revérifié : 12/12 en local (config Debug) **et** 12/12 sur le runner GitHub Actions (config Release, `arkcloud-backend-ci.yml`) — les deux environnements de build confirmés, pas seulement l'un des deux.

**infracost — toujours non vérifié par un run réel** : contrairement à ArchUnitNET ci-dessus, ce job n'a encore jamais tourné en conditions réelles — bloqué sur `INFRACOST_API_KEY`, pas encore posé (voir setup requis ci-dessus). S'appuie sur la syntaxe documentée officiellement (`github.com/infracost/actions`) plutôt que sur une exécution réelle. À tester avant de considérer ce point clos : une vraie PR sur `ArkCloudInfra` une fois le secret posé — vu ce qui vient d'arriver côté ArchUnitNET, ne pas supposer que la syntaxe documentée suffit tant que ça n'a pas tourné au moins une fois.
