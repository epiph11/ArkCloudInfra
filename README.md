...
**Reproductibilité du build Lambda entre local (Windows/Git Bash) et CI (Ubuntu)** : `filebase64sha256()` sur le zip signifie que deux constructions du même code doivent produire le même hash, sous peine de voir Terraform considérer la Lambda comme modifiée à chaque alternance local/CI — même classe de problème que le flip-flop de security group ci-dessus. Deux fausses pistes avant la vraie cause, les deux tracées ici plutôt que silencieusement corrigées : (1) fins de ligne CRLF sur `rotate.py` — fix appliqué (`.gitattributes`, `tr -d '\r'`) sans le moindre effet sur le hash, `rotate.py` n'a jamais divergé ; (2) un `terraform plan` vide observé en CI, pris à tort pour une preuve de convergence — il comparait en réalité le build de la CI à un state que la CI elle-même avait posé, pas au build local, donc ne prouvait rien sur la divergence local/CI. La vraie cause, trouvée en comparant le manifeste par fichier local à celui publié par la CI (`build/manifest.txt`, un hash SHA-256 par fichier archivé, généré par `build.sh` — voir `modules/aws/secret-rotation/lambda/README.md`) : un seul fichier différait sur 34, `psycopg2_binary-2.9.10.dist-info/RECORD` — pas du contenu PyPI, mais une métadonnée que pip génère lui-même à l'installation et formate différemment selon sa version. Fixé en supprimant tout `*.dist-info` du package (rien dedans n'est lu par le runtime Lambda), pas en essayant de neutraliser ce fichier précis. Leçon générale : la preuve de reproductibilité qui compte est la comparaison directe des deux builds (le manifeste), pas un plan Terraform qui peut être vide pour une mauvaise raison.
