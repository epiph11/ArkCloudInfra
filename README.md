...
**Bug réel rencontré lors d'une rotation (Sprint 6)** : après un `terraform apply` confirmé (`docker_registry_password` détecté comme changé, poussé sans erreur) puis un restart, `app-arkcloud-api-dev` continuait à échouer avec `ImagePullUnauthorizedFailure` — persistant même après un nouvel apply qui affichait ensuite `No changes` (l'état correspondait bien à la config voulue). Seul un `az webapp config container set` en CLI (API différente, plus ancienne) a réellement débloqué le pull. Root-caused, pas supposé : bug confirmé du provider `azurerm` ([`hashicorp/terraform-provider-azurerm#22996`](https://github.com/hashicorp/terraform-provider-azurerm/issues/22996), même classe que `#23525`) — les arguments `application_stack.docker_registry_*` ne se propagent pas de façon fiable au sous-système réel de pull de l'App Service. Corrigé à la racine dans `modules/azure/app-service/main.tf` : les trois `app_settings` classiques (`DOCKER_REGISTRY_SERVER_URL`/`_USERNAME`/`_PASSWORD`) sont désormais aussi posés explicitement par Terraform, en plus du bloc `application_stack` — c'est le contournement documenté par l'issue amont. Un futur `terraform apply` régulier suffit donc de nouveau ; le `az webapp config container set` manuel ne devrait plus être nécessaire.
